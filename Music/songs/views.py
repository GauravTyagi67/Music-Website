...
def home(request):
    ...
    return render(request, 'songs/home.html',{'albums':albums})

def profile_detail(request, username):
    # show all albums of the artist
    albums = get_object_or_404(User, username=username)
    albums = albums.albums.all()
    return render(request, 'songs/artist_detail.html', {'albums':albums, 'username':username})
    ...
    if user == request.user:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect('songs:profile_detail', username=request.user)
        else:
            form = NewAlbum()
        return render(request, 'songs/add_new_album.html', {'form':form})
    else:
        return redirect('songs:profile_detail', username=user)
...
def album_detail(request,username, album):
    ...
    return render(request, 'songs/album_detail.html', {'songs':songs, 'album':album, 'username':username})
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'songs/signup.html', {'form':form,'message':message})
    else:
        form = SignUpForm()
...
def delete_album(request, username, album):
    ...
    if request.user == user:
        ...
        return redirect('songs:profile_detail', username=username)
    else:
        return redirect('songs:profile_detail', username=username)
    ...
    if request.user == user:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect('songs:album_detail', username=username, album=album)

        else:
            form = NewSong()
            return render(request, 'songs/add_new_song.html', {'form':form})
    else:
        return redirect('songs:album_detail', username=username, album=album)